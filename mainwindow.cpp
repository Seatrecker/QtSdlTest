...
void MainWindow::closeJoystick()
{
    ...
    QLayoutItem * child;
    while ((child = ui->buttonsLayout->takeAt(0)) != 0) {
        delete child;
    }
    ...
    ui->choosePadBox->setItemText(0, "None");
}
...
void MainWindow::setJoystick(_SDL_Joystick *joystick)
{
    closeJoystick();
    ...
    qDebug("%s: setup joystick %d %s", __CLASS__,
           SDL_JoystickInstanceID(joystick), SDL_JoystickName(joystick));
    ui->choosePadBox->setItemText(0, SDL_JoystickName(joystick));
...
{
    if (!joystick)
        setJoystick(SDL_JoystickOpen(event.which));
}
